...
func (l *UnixDatagramSocketListener) listen(running bool, out chan []byte, closer chan bool) func(pipe chan []byte) {
	return func(pipe chan []byte) {
		b := make([]byte, 4096)
		for {
			if !running {
				break
